...
interface NotificationBellContextValue {
  api: ApiLike;
  useAuthUser: () => { id?: string };
}

export const NotificationBellContext = createContext<NotificationBellContextValue | null>(null);
...
export function NotificationBellProvider({
  ...
  return (
    <NotificationBellContext.Provider value={{ api, useAuthUser }}>
      {children}
...
export type { AppNotification, NotificationRole, NotificationType };
...
export function NotificationBell({
  ...
  if (!ctx) {
    throw new Error("NotificationBell must be used within a NotificationBellProvider");
  }
  const { api, useAuthUser } = ctx;

  const navigate = useNavigate();
  const { id } = useAuthUser();

  const hooksRef = useRef(createNotificationHooks(api));
  const { useNotifications } = hooksRef.current;
  ...
  useEffect(() => {
    if (usePortal && open && btnRef.current) {
