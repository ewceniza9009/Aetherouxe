...
import { Home, Layers, Car, CheckCircle, Sparkles, X, Bed, Bath, Maximize } from 'lucide-react';
...
const TownhouseUnitMesh: React.FC<TownhouseUnitMeshProps> = ({
  ...
  useFrame((_, delta) => {
    ...
    if (isHovered || isSelected) {
      ...
      groupRef.current.scale.lerp(new THREE.Vector3(1.04, 1.04, 1.04), delta * 10);
    } else {
  ...
  return (
    ...
    >
      ...
      {/* Carport Door Inset */}
      <RoundedBox args={[1.4, 0.95, 0.1]} radius={0.02} position={[0, 0.5, 1.62]}>
        <meshStandardMaterial color="#1e293b" roughness={0.5} metalness={0.8} />
      </RoundedBox>

      {/* Second Floor Living & Balcony */}
      <RoundedBox args={[1.96, 1.3, 3.0]} radius={0.04} smoothness={3} position={[0, 1.95, 0.1]}>
        <meshStandardMaterial
          ...
          metalness={0.7}
          emissive={isHovered || isSelected ? colorInfo.main : '#0a0f1d'}
          emissiveIntensity={isHovered || isSelected ? 0.85 : 0.2}
        />
      </RoundedBox>
      ...
      {/* Private Driveway Apron */}
      <mesh position={[0, 0.02, 2.4]} rotation={[-Math.PI / 2, 0, 0]}>
...
export const TownhouseClusterView: React.FC<TownhouseClusterProps> = ({
  ...
  return (
    ...
    >
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/30">
                Townhouse Row Cluster
              </span>
            </div>
            <p className="text-xs text-slate-400">
              {totalCount} Attached 2-Story Residential Units • Private Garage & Balcony
            </p>
          </div>
        </div>
      ...
      {/* Main 3D Canvas Area */}
      <div
        className="relative w-full h-[480px] bg-gradient-to-b from-[#070b14] via-[#0b1329] to-[#050811] overflow-hidden"
        onMouseMove={(e) => {
          const rect = e.currentTarget.getBoundingClientRect();
          setMousePos({ x: e.clientX - rect.left, y: e.clientY - rect.top });
        }}
      >
        <Canvas camera={{ position: [0, 4.5, clusterWidth * 0.7 + 4], fov: 38 }} shadows>
          ...
          <group position={[0, 0, 0]}>
            {/* Street / Driveway Asphalt */}
            <mesh position={[0, -0.01, 3.8]} rotation={[-Math.PI / 2, 0, 0]}>
              <planeGeometry args={[clusterWidth + 12, 5]} />
              <meshStandardMaterial color="#0b1120" roughness={0.9} />
            </mesh>

            {/* Road Centerline */}
            <mesh position={[0, 0.01, 4.2]} rotation={[-Math.PI / 2, 0, 0]}>
              <planeGeometry args={[clusterWidth + 10, 0.1]} />
              <meshStandardMaterial color="#f59e0b" roughness={0.5} />
            </mesh>

            {/* Main Landscaped Parcel Ground */}
            <RoundedBox
              args={[clusterWidth + 4, 0.15, 7.5]}
              radius={0.08}
              position={[0, -0.1, 0.5]}
            >
