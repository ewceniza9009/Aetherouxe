...
const TYPE_BADGE: Record<string, { label: string; cls: string }> = {
  ...
  spot_cash: { label: "Spot Cash", cls: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20" },
  installment: { label: "Installment", cls: "bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20" },
  mortgage_assisted: { label: "Mortgage", cls: "bg-violet-500/10 text-violet-600 dark:text-violet-400 border-violet-500/20" },
  rent_to_own: { label: "RTO", cls: "bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border-cyan-500/20" },
};
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">Select the payment scheme to apply to a unit.</p>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between mb-4">
                        <span className="text-lg font-bold group-hover:text-primary transition-colors">{t.code}</span>
                        <Badge variant="outline" className={`text-[10px] shrink-0 ${badge.cls}`}>{badge.label}</Badge>
                      </div>
                      {t.name && <p className="text-sm text-muted-foreground line-clamp-2 mb-4">{t.name}</p>}
                      <Separator className="mb-4" />
                      <div className="flex flex-wrap gap-x-5 gap-y-2 text-xs text-muted-foreground">
                        {t.agentCommissionPercentage && <span>{t.agentCommissionPercentage}% commission</span>}
                        {t.penaltyPercent && <span>{t.penaltyPercent}% penalty</span>}
                        {t.graceDays && <span>{t.graceDays}d grace</span>}
                        {t.mortgageDownPaymentPercent && <span>{t.mortgageDownPaymentPercent}% DP</span>}
                    ...
                    <tbody>
                      {availableUnits.map((u: any) => (
                        <tr key={u.id} className="border-b border-border/30 hover:bg-muted/20 transition-colors">
                          ...
                          <td className="px-6 py-4 text-sm capitalize">{(u.type ?? u.unitType ?? "—").replace(/_/g, " ")}</td>
                          <td className="px-6 py-4 text-right tabular-nums text-xs text-muted-foreground">
                            {u.size ? `${u.size}m²` : "—"}
                          </td>
                          <td className="px-6 py-4 text-center text-xs tabular-nums">{u.bedrooms ?? 0}</td>
                          <td className="px-6 py-4 text-center text-xs tabular-nums">{u.bathrooms ?? 0}</td>
                          <td className="px-6 py-4 text-right font-semibold text-sm tabular-nums">
                            {u.listPrice ? formatCurrency(u.listPrice) : "—"}
                          </td>
                          <td className="px-6 py-4 text-right">
                            <Button size="default" className="h-9 px-5 text-sm" onClick={() => selectUnit(u)}>
                              Select
          ...
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Assign buyer & agent to <span className="font-semibold text-primary">{selectedUnit?.unitNumber}</span>
                </p>
              </div>
              <Button variant="ghost" size="default" onClick={goBack}>
                <ArrowLeft className="h-4 w-4 mr-2" />
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    {selectedTemplate?.penaltyPercent && (
                      <div>
                        ...
                        <span className="font-semibold">{selectedTemplate.penaltyPercent}%/day</span>
                      </div>
                    )}
                    ...
                    {selectedTemplate?.agentCommissionPercentage && (
                      <div>
                        ...
                        <span className="font-semibold">{selectedTemplate.agentCommissionPercentage}%</span>
                      </div>
                    )}
                    {selectedTemplate?.mortgageDownPaymentPercent && (
                      <div>
                        <span className="text-muted-foreground block text-xs mb-1">Down Payment</span>
                        <span className="font-semibold">{selectedTemplate.mortgageDownPaymentPercent}%</span>
                      </div>
                    )}
                    {selectedTemplate?.interestRatePercent && (
                      <div>
                        <span className="text-muted-foreground block text-xs mb-1">Interest Rate</span>
                        <span className="font-semibold">{selectedTemplate.interestRatePercent}%</span>
                      </div>
                    )}
                    {selectedTemplate?.loanTermMonths && (
                      <div>
                        <span className="text-muted-foreground block text-xs mb-1">Loan Term</span>
                        <span className="font-semibold">{selectedTemplate.loanTermMonths} months</span>
                      </div>
                    )}
                  </div>
                </CardContent>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <span className="font-semibold">{selectedUnit?.property?.propertyCode ?? "—"}</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground block text-xs mb-1">Type</span>
                      <span className="font-semibold capitalize">{(selectedUnit?.type ?? selectedUnit?.unitType ?? "—").replace(/_/g, " ")}</span>
                    </div>
                    <div>
                      <span className="text-muted-foreground block text-xs mb-1">Size</span>
                      <span className="font-semibold">{selectedUnit?.size ? `${selectedUnit.size}m²` : "—"}</span>
                    </div>
                  </div>
                  {selectedUnit?.listPrice && (
                    <>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                        <div>
                          <span className="text-muted-foreground block text-xs mb-1">List Price</span>
                          <span className="font-bold text-base">{formatCurrency(selectedUnit.listPrice)}</span>
                        </div>
                        {selectedUnit?.lotValue != null && (
                          <div>
                            <span className="text-muted-foreground block text-xs mb-1">Lot Value</span>
                            <span className="font-semibold">{formatCurrency(selectedUnit.lotValue)}</span>
                          </div>
                        )}
                        {selectedUnit?.buildingValue != null && (
                          <div>
                            <span className="text-muted-foreground block text-xs mb-1">Building Value</span>
                            <span className="font-semibold">{formatCurrency(selectedUnit.buildingValue)}</span>
                          </div>
                        )}
                      </div>
                    </>
                      ...
                      <SelectContent>
                        {agents.map((a: any) => (
                          <SelectItem key={a.id} value={a.id} className="text-sm">
                            {a.user?.firstName ? [a.user.firstName, a.user.lastName].filter(Boolean).join(" ") : a.userId}
        ...
        {step === 3 && result && (
          <div className="space-y-8 max-w-2xl mx-auto">
                ...
                <div className="grid grid-cols-2 gap-5 text-sm">
                  {result.leaseId && (
                    <div className="rounded-xl bg-muted/30 p-4">
                      <span className="text-muted-foreground text-xs block mb-1.5">Lease Agreement</span>
                      <span className="font-semibold text-base">Created</span>
                    </div>
                  )}
                  {result.mortgage?.periods && (
                    <div className="rounded-xl bg-muted/30 p-4">
                      <span className="text-muted-foreground text-xs block mb-1.5">Mortgage Scenario</span>
                      <span className="font-semibold text-base">{result.mortgage.periods} amortization periods</span>
                    </div>
                  )}
                  {result.rtoContractId && (
                    <div className="rounded-xl bg-muted/30 p-4">
                      <span className="text-muted-foreground text-xs block mb-1.5">RTO Contract</span>
                  ...
                  {result.invoice && (
                    <div className="rounded-xl bg-muted/30 p-4">
                      ...
                      <span className="font-semibold text-base ml-2">{formatCurrency(result.invoice.amount)}</span>
                    </div>
                  )}
                  {result.agentTransaction && (
                    <div className="col-span-2 rounded-xl bg-muted/30 p-4">
                      <span className="text-muted-foreground text-xs block mb-1.5">Agent Commission</span>
                      <span className="font-semibold text-base">{formatCurrency(result.agentTransaction.calculatedCommission)}</span>
                      <span className="text-muted-foreground ml-2">({result.agentTransaction.commissionPercent}%)</span>
                    </div>
                  )}
                </div>
              </CardContent>
            ...
            <div className="flex justify-center gap-4">
              <Button variant="outline" size="lg" onClick={reset}>Apply Another</Button>
              <Button size="lg" onClick={() => navigate({ to: "/" })}>Back to Dashboard</Button>
            </div>
          </div>
        )}
