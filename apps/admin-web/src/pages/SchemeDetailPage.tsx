          ...
          <SectionCard title="Rental Terms" color="rgba(59,130,246,0.7)">
            <div className="grid gap-3 sm:grid-cols-3">
              <Field
                ...
                value={form.securityDepositPercent}
                onChange={(v) => set('securityDepositPercent', v)}
              />
              <Field
                label="Penalty % per day"
                value={form.penaltyPercent}
                onChange={(v) => set('penaltyPercent', v)}
              />
              <Field
                label="Grace Days"
          ...
          <>
            <SectionCard title="Downpayment" color="rgba(59,130,246,0.7)">
              <div className="grid gap-3 sm:grid-cols-3">
                <Field
                  label="# Payments"
                  value={form.dpNumberOfPayments}
                  onChange={(v) => set('dpNumberOfPayments', v)}
                />
                <Field
                  label="Days from DP"
                  value={form.dpNumberOfDaysFromDp}
                  onChange={(v) => set('dpNumberOfDaysFromDp', v)}
                />
                <Field
                  label="Amount"
                  value={form.dpAmount}
                  onChange={(v) => set('dpAmount', v)}
                  step="0.01"
                />
              </div>
              <TextField
                label="Remarks"
                value={form.dpRemarks}
                onChange={(v) => set('dpRemarks', v)}
                rows={2}
              />
            </SectionCard>
            ...
            <SectionCard title="Equity" color="rgba(34,197,94,0.7)">
              <div className="grid gap-3 sm:grid-cols-3">
                <Field
                  label="# Payments"
                  value={form.eqNumberOfPayments}
                  onChange={(v) => set('eqNumberOfPayments', v)}
                />
                <Field
                  label="Days from DP"
                  value={form.eqNumberOfDaysFromDp}
                  onChange={(v) => set('eqNumberOfDaysFromDp', v)}
                />
                <Field
                  label="Amount"
                  value={form.eqAmount}
                  onChange={(v) => set('eqAmount', v)}
                  step="0.01"
                />
                <Field
                  label="EQ Payment %"
                  value={form.eqPaymentPercentage}
                  onChange={(v) => set('eqPaymentPercentage', v)}
                  step="0.0001"
                />
                <Field
                  label="EQ DP %"
                  value={form.eqDownpaymentPercentage}
                  onChange={(v) => set('eqDownpaymentPercentage', v)}
                  step="0.0001"
                />
                <Field
                  label="EQ Monthly Amort %"
                  value={form.eqMonthlyAmortPercentage}
                  onChange={(v) => set('eqMonthlyAmortPercentage', v)}
                  step="0.0001"
                />
                <Field
                  label="EQ Discount %"
                  value={form.eqDiscountPercentage}
                  onChange={(v) => set('eqDiscountPercentage', v)}
                  step="0.0001"
                />
                <Field
                  label="Order Sequence #"
                  value={form.eqPaymentOrderNumber}
                  onChange={(v) => set('eqPaymentOrderNumber', v)}
                />
              </div>
              <TextField
                label="Remarks"
                value={form.eqRemarks}
                onChange={(v) => set('eqRemarks', v)}
                rows={2}
              />
            </SectionCard>
            ...
            <SectionCard title="Balance" color="rgba(239,68,68,0.7)">
              <div className="grid gap-3 sm:grid-cols-3">
                <Field
                  label="# Payments"
                  value={form.blNumberOfPayments}
                  onChange={(v) => set('blNumberOfPayments', v)}
                />
                <Field
                  label="Days from DP"
                  value={form.blNumberOfDaysFromDp}
                  onChange={(v) => set('blNumberOfDaysFromDp', v)}
                />
                <Field
                  label="Amount"
                  value={form.blAmount}
                  onChange={(v) => set('blAmount', v)}
                  step="0.01"
                />
                <Field
                  label="BL Payment %"
                  value={form.blPaymentPercentage}
                  onChange={(v) => set('blPaymentPercentage', v)}
                  step="0.0001"
                />
                <Field
                  label="Misc %"
                  value={form.blMiscPercentage}
                  onChange={(v) => set('blMiscPercentage', v)}
                  step="0.0001"
                />
              </div>
              <div className="flex gap-4">
                <label className="flex items-center gap-2 text-sm">
                  <Checkbox
                    checked={form.blIsChangeOrder ?? false}
                    onCheckedChange={(checked) => set('blIsChangeOrder', checked === true)}
                  />
                  Change Order
                </label>
                <label className="flex items-center gap-2 text-sm">
                  <Checkbox
                    checked={form.blIncludeDpAmort ?? false}
                    onCheckedChange={(checked) => set('blIncludeDpAmort', checked === true)}
                  />
                  Include DP Amort
                </label>
              </div>
              <TextField
                label="Remarks"
                value={form.blRemarks}
                onChange={(v) => set('blRemarks', v)}
                rows={2}
              />
            </SectionCard>
          </>
        ...
        {schemeType === 'mortgage_assisted' && (
          <SectionCard title="Mortgage Terms" color="rgba(168,85,247,0.7)">
            <div className="grid gap-3 sm:grid-cols-3">
              <Field
                label="Down Payment %"
                value={form.mortgageDownPaymentPercent}
                onChange={(v) => set('mortgageDownPaymentPercent', v)}
                step="0.01"
              />
              <Field
                label="Interest Rate %"
        ...
        {schemeType === 'rent_to_own' && (
          <SectionCard title="RTO Terms" color="rgba(244,63,94,0.7)">
            <div className="grid gap-3 sm:grid-cols-3">
              <Field
                label="Option Fee %"
                value={form.optionFeePercent}
                onChange={(v) => set('optionFeePercent', v)}
                step="0.01"
              />
              <Field
                label="Equity Accumulation %"
                value={form.equityAccumulationPercent}
                onChange={(v) => set('equityAccumulationPercent', v)}
                step="0.01"
              />
              <Field
                label="Target Purchase (years)"
                value={form.targetPurchaseYears}
                onChange={(v) => set('targetPurchaseYears', v)}
              />
            </div>
          </SectionCard>
        )}
          ...
          <SectionCard title="Commissions" color="rgba(168,85,247,0.7)">
            <div className="grid gap-3 sm:grid-cols-2">
              <Field
                ...
                onChange={(v) => set('agentCommissionPercentage', v)}
                step="0.0001"
              />
              <Field
                label="Company %"
                value={form.companyCommissionPercentage}
                onChange={(v) => set('companyCommissionPercentage', v)}
                step="0.0001"
              />
            </div>
          </SectionCard>
