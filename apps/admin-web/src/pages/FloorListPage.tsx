...
export default function FloorListPage() {
  ...
  const handleMoveFloor = useCallback(
    async (floor: Floor, direction: 'up' | 'down') => {
      if (!floors) return;
    ...
    <div className="space-y-4 flex flex-col h-[calc(100vh-6rem)] min-h-0">
      <div className="flex items-center gap-4 shrink-0">
        <Button
          ...
          className="bg-background/50 hover:bg-muted shadow-sm"
        >
        ...
        <Avatar
          src={
            building?.images?.length
              ? building.images.find((img) => img.isPrimary)?.url || building.images[0].url
              : undefined
          }
          alt={building?.name}
          fallback={<Building2 className="h-6 w-6" />}
          rounded="lg"
          className="h-12 w-12 ring-2 ring-border/60 shadow-elevate"
        />
        <div>
        ...
        <div className="bg-muted/10 p-4 border-b border-border/40">
          <div className="flex items-end gap-3">
            ...
            <Button
              ...
              disabled={!newFloorNumber || createFloor.isPending}
              ...
              {sortedFloors.map((floor, idx) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-medium text-muted-foreground mt-0.5">
                        Sort order: {floor.sortOrder} &middot; {floor.unitsCount ?? 0} unit
                        {(floor.unitsCount ?? 0) !== 1 ? 's' : ''}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge
                      variant="secondary"
                      className="px-2 font-bold bg-muted/50 text-muted-foreground"
                    >
                      {floor.unitsCount ?? 0} units
                    </Badge>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete Floor {deleteTarget?.floorNumber}? This cannot be
              undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
