...
export type AgentTierValue = 'junior' | 'senior' | 'team_lead' | 'external_broker';
...
export function useAgents(query: AgentQuery) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (query.isInternal !== undefined) params.set('isInternal', String(query.isInternal));
...
export function useAgentPerformance(id: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const { data } = await api.get<ApiResponse<AgentPerformance>>(`/agents/${id}/performance`);
