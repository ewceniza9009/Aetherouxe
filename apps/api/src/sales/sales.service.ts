...
const nnum = (v: any): number => { const n = Number(v); return isNaN(n) ? 0 : n; };
...
export class SalesService {
  ...
  async applyScheme(dto: ApplySchemeDto, performedByUserId?: string) {
    ...
    const termYears = scheme.schemeType === 'rent_to_own'
      ? nnum(scheme.targetPurchaseYears) || 5
      : 1;
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      switch (scheme.schemeType) {
        case 'installment':
          await this.generateInstallmentInvoices(tx, scheme, lease, unit, value, startDate, tenantId, buyer.id, result);
          break;
        case 'mortgage_assisted':
          await this.generateMortgageInvoices(tx, scheme, lease, unit, dto, value, startDate, tenantId, buyer.id, result);
          break;
        case 'rent_to_own':
          await this.generateRtoRecords(tx, scheme, lease, unit, dto, value, startDate, tenantId, buyer.id, performedByUserId, result);
          break;
        case 'spot_cash':
          await this.generateSpotCashInvoice(tx, scheme, lease, unit, value, startDate, tenantId, buyer.id, result);
          break;
  ...
  private mapLeaseType(schemeType: string): string {
    switch (schemeType) {
      case 'rent_to_own': return 'rent_to_own';
      case 'corporate_lease': return 'corporate_lease';
      default: return 'standard_rental';
    }
  }

  private mapTransactionType(schemeType: string): string {
    switch (schemeType) {
      case 'rent_to_own': return 'rto_contract';
      case 'standard_rental': return 'rental_lease';
      default: return 'sale';
    }
  }
  ...
  private async generateInstallmentInvoices(
    tx: any, scheme: any, lease: any, unit: any, value: number,
    startDate: Date, tenantId: string, buyerId: string, result: any,
  ) {
    const invoices: any[] = [];
    ...
    const dpTotal = dpAmount > 0 ? dpAmount : round2(value * (nnum(scheme.eqDownpaymentPercentage) || 20) / 100);
    if (dpPayments > 0 && dpTotal > 0) {
      const dpPerMonth = round2(dpTotal / dpPayments);
      for (let i = 0; i < dpPayments; i++) {
        const dueDate = new Date(startDate);
        dueDate.setMonth(dueDate.getMonth() + i + 1);
        const inv = await this.createInvoice(tx, tenantId, buyerId, dpPerMonth, dueDate,
          'downpayment', `DP installment ${i + 1}/${dpPayments} for ${unit.unitNumber}`, lease.id);
        invoices.push(inv);
      }
    }
    ...
    if (eqPayments > 0 && eqTotal > 0) {
      ...
      for (let i = 0; i < eqPayments; i++) {
        ...
        const inv = await this.createInvoice(tx, tenantId, buyerId, eqPerMonth, dueDate,
          'equity_credit', `EQ installment ${i + 1}/${eqPayments} for ${unit.unitNumber}`, lease.id);
        invoices.push(inv);
      }
    }
    ...
    if (blTotal > 0) {
      ...
      const inv = await this.createInvoice(tx, tenantId, buyerId, blTotal, dueDate,
        'downpayment', `Balance payment for ${unit.unitNumber}`, lease.id);
      invoices.push(inv);
    }
  ...
  private async generateMortgageInvoices(
    tx: any, scheme: any, lease: any, unit: any, dto: ApplySchemeDto,
    value: number, startDate: Date, tenantId: string, buyerId: string, result: any,
  ) {
    ...
    if (dpPayments > 0 && dpTotal > 0) {
      ...
      result.dpInvoices = invoices.map((inv) => ({
        id: inv.id, invoiceNumber: inv.invoiceNumber, amount: Number(inv.amount),
      }));
    } else if (dpTotal > 0) {
      // Single DP invoice
      const dueDate = new Date(startDate);
      dueDate.setDate(dueDate.getDate() + 15);
      const inv = await this.createInvoice(tx, tenantId, buyerId, dpTotal, dueDate,
        'downpayment', `Down payment (${dpPct}%) for ${unit.unitNumber}`, lease.id);
      result.dpInvoice = { id: inv.id, invoiceNumber: inv.invoiceNumber, amount: Number(inv.amount) };
    }
    ...
    if (loanAmount > 0) {
      ...
      await tx.mortgageAmortizationSchedule.createMany({
        data: rows.map((r) => ({
          ...
          periodDate: new Date(new Date(startDate).setMonth(startDate.getMonth() + (r.periodNumber - 1))),
        })),
      });
  ...
  private async generateRtoRecords(
    tx: any, scheme: any, lease: any, unit: any, dto: ApplySchemeDto,
    value: number, startDate: Date, tenantId: string, buyerId: string,
    performedByUserId: string | undefined, result: any,
  ) {
    ...
    const rto = await tx.rtoContract.create({
      data: {
        ...
        accumulatedEquity: 0,
        targetPurchaseDate: new Date(new Date().setFullYear(new Date().getFullYear() + nnum(scheme.targetPurchaseYears) || 5)),
    ...
    if (optionFee > 0) {
      ...
      const dueDate = new Date(startDate);
      dueDate.setDate(dueDate.getDate() + 15);
      const inv = await this.createInvoice(tx, tenantId, buyerId, optionFee, dueDate,
        'reservation', `RTO option fee for ${unit.unitNumber}`, lease.id);
  ...
  private async generateSpotCashInvoice(
    ...
    const dueDate = new Date(startDate);
    dueDate.setDate(dueDate.getDate() + 15);
    const inv = await this.createInvoice(tx, tenantId, buyerId, invoiceAmount, dueDate,
      'downpayment', `Spot cash payment for ${unit.unitNumber}${discountPct > 0 ? ` (${discountPct}% discount)` : ''}`, lease.id);
    result.invoice = { id: inv.id, invoiceNumber: inv.invoiceNumber, amount: Number(inv.amount) };
    if (discountPct > 0) {
  ...
  async listSchemes(): Promise<SchemeListItem[]> {
    ...
    return (leases as any[]).map((l): SchemeListItem => ({
      ...
      agentName: l.agent?.user
        ? [l.agent.user.firstName, l.agent.user.lastName].filter(Boolean).join(' ') || l.agent.user.email
        : null,
