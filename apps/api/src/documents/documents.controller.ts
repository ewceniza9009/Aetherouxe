...
export class DocumentsController {
  ...
  @ApiOperation({ summary: 'List documents for an owner' })
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get a document by ID (includes signatures)' })
  ...
  @Get(':id/signatures')
  @ApiOperation({ summary: 'List signatures for a document' })
  findSignatures(@Param('id') id: string) {
    return this.service.findAllSignatures(id);
  }

  @Post(':id/signatures')
  @ApiOperation({ summary: 'Create a signature request for a document' })
  createSignature(@Param('id') id: string, @Body() dto: CreateSignatureDto) {
    return this.service.createSignature({ ...dto, documentVaultId: id });
  }

  @Post('signatures/:sigId/mark-signed')
  @ApiOperation({ summary: 'Mark a signature as signed' })
  markSigned(@Param('sigId') sigId: string) {
    return this.service.markSignatureSigned(sigId);
  }

  @Post(':id/mark-signed')
  @ApiOperation({ summary: 'Mark a documents signatures as signed' })
  markDocumentSigned(@Param('id') id: string) {
    return this.service.markDocumentSigned(id);
  }

  @Get('signatures/:sigId')
  @ApiOperation({ summary: 'Get a signature by ID' })
  findOneSignature(@Param('sigId') sigId: string) {
    return this.service.findOneSignature(sigId);
  }
  ...
  removeSignature(@Param('sigId') sigId: string) {
    return this.service.removeSignature(sigId);
  }
}
