...
export class FloorsController {
  ...
  @Get()
  @ApiOperation({ summary: 'List floors by building' })
  findByBuilding(
  ...
  @ApiOperation({ summary: 'List floors by building (nested)' })
  findByBuildingNested(
    @Request() req: ExpressRequest & { user: { tenantId: string } },
  ...
  @Roles(UserType.super_admin, UserType.admin, UserType.property_manager)
  @ApiOperation({ summary: 'Create a floor (nested)' })
  createNested(
    @Request() req: ExpressRequest & { user: { tenantId: string } },
    @Param('buildingId') buildingId: string,
    @Body() dto: CreateFloorDto,
  ) {
    return this.service.create({ ...dto, buildingId } as CreateFloorDto, req.user.tenantId);
  }

  @Patch('buildings/:buildingId/floors/:id')
  @Roles(UserType.super_admin, UserType.admin, UserType.property_manager)
  @ApiOperation({ summary: 'Update a floor (nested)' })
  updateNested(
    @Request() req: ExpressRequest & { user: { tenantId: string } },
    @Param('id') id: string,
  ...
  @Roles(UserType.super_admin, UserType.admin, UserType.property_manager)
  @ApiOperation({ summary: 'Delete a floor (nested)' })
  removeNested(
    @Request() req: ExpressRequest & { user: { tenantId: string } },
    @Param('id') id: string,
  ) {
